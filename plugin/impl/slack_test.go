...
var _ = Describe("The slack thread plugin", func() {
	It("should parse its config", func() {
		...
		Expect(plugin.(*SlackThread).Lease.Name.Name).To(Equal("lease"))
		Expect(plugin.(*SlackThread).Lease.Name.Namespace).To(Equal("default"))
		Expect(plugin.(*SlackThread).Lease.Period).To(Equal(time.Minute))
	})
})
